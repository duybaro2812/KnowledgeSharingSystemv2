...
function PreviewPanel(props) {
  ...
  const ratingSummary = getDocRatings
    ? getDocRatings(docId)
    : { totalRatings: 0, averageStars: 0, distribution: {}, myRating: null, recentRatings: [] };
  ...
  const canRateDocument =
    Number.isInteger(docId) &&
    docId > 0 &&
    Number(currentUserId) > 0 &&
    !isOwner &&
    Boolean(onSaveRating);
  ...
  const handleSaveRating = async () => {
    if (!canRateDocument || !onSaveRating || isSubmittingRating) return;
    const parsedStars = Number(ratingStars);
    if (!Number.isInteger(parsedStars) || parsedStars < 1 || parsedStars > 5) return;
  ...
  const handleDeleteRating = async () => {
    if (!ratingSummary?.myRating || !onDeleteRating || isSubmittingRating) return;
    setIsSubmittingRating(true);
    try {
      await onDeleteRating(docId);
    } finally {
      setIsSubmittingRating(false);
    }
  };

  const renderStars = (value, className = "") => {
    const normalized = Math.max(0, Math.min(5, Number(value || 0)));
    return (
      <span className={`document-rating-stars ${className}`} aria-label={`${normalized}/5 stars`}>
        {[1, 2, 3, 4, 5].map((star) => (
          <span key={`display-star-${star}`} className={star <= Math.round(normalized) ? "filled" : ""}>
            ★
          </span>
        ))}
      </span>
  ...
  return (
    <section className="panel preview-panel">
      ...
      <section className="document-rating-panel">
        <div className="document-rating-summary">
          <div>
            <span className="document-rating-kicker">Đánh giá tài liệu</span>
            <h3>{Number(ratingSummary.averageStars || 0).toFixed(1)}/5</h3>
            {renderStars(ratingSummary.averageStars, "document-rating-stars-large")}
            <p>{Number(ratingSummary.totalRatings || 0)} lượt đánh giá từ người học</p>
          </div>
          <div className="document-rating-breakdown" aria-label="Rating breakdown">
            {[5, 4, 3, 2, 1].map((star) => {
              const count = Number(ratingSummary.distribution?.[star] || 0);
              const total = Number(ratingSummary.totalRatings || 0);
              const percent = total > 0 ? Math.round((count / total) * 100) : 0;
              return (
                <div className="document-rating-bar-row" key={`rating-row-${star}`}>
                  <span>{star} sao</span>
                  <div className="document-rating-bar">
        ...
        <div className="document-rating-compose">
          {isGuestLike ? (
            <div className="document-rating-login-card">
              <strong>Đăng nhập để đánh giá tài liệu</strong>
              <p>Đánh giá giúp người học khác chọn tài liệu phù hợp hơn.</p>
            ...
            <div className="document-rating-login-card">
              <strong>Bạn là tác giả tài liệu này</strong>
              <p>Tác giả không tự đánh giá tài liệu của mình để giữ kết quả khách quan.</p>
            </div>
          ) : (
            <>
              <div className="document-rating-compose-head">
                <strong>{ratingSummary.myRating ? "Cập nhật đánh giá của bạn" : "Đánh giá tài liệu này"}</strong>
                {ratingSummary.myRating && <span>Đã đánh giá {ratingSummary.myRating.stars}/5 sao</span>}
              </div>
              <div className="document-rating-input-stars" role="radiogroup" aria-label="Chọn số sao">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    type="button"
                    key={`rating-input-${star}`}
                    className={star <= Number(ratingStars || 0) ? "active" : ""}
                    aria-label={`${star} sao`}
              ...
              <div className="document-rating-compose-actions">
                ...
                {ratingSummary.myRating && (
                  <button
                    type="button"
                    className="document-rating-remove-btn"
                    disabled={isBusy || isSubmittingRating}
                ...
                <button
                  ...
                  disabled={!canRateDocument || !ratingStars || isBusy || isSubmittingRating}
        ...
        <div className="document-rating-list">
          {(ratingSummary.recentRatings || []).length === 0 ? (
            <p className="document-rating-empty">Chưa có đánh giá nào cho tài liệu này.</p>
          ) : (
            (ratingSummary.recentRatings || []).map((item) => (
              <article className="document-rating-item" key={`doc-rating-${item.ratingId}`}>
                <div className="document-rating-avatar">
                  {String(item.userName || "U").slice(0, 1).toUpperCase()}
                </div>
                <div>
                  <div className="document-rating-item-head">
