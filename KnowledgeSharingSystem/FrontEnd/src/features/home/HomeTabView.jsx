        ...
        {docs.map((doc, index) => (
          <article key={doc.documentId} className="home-gentle-card">
            <button type="button" className="reading-card-click" onClick={() => onOpenDoc(doc)}>
              <span className="sr-only">{doc.openLabel || "Open document"} {doc.title}</span>
            </button>
    ...
    <section className="home-gentle-section">
      <div className="home-gentle-section-head">
        <div>
          <h2>
            <BookIcon />
            {text.topCourses || "Top Courses"}
          </h2>
          <p>{text.topCoursesSubtitle || "Popular course areas with active materials and discussion"}</p>
        </div>
      </div>
      ...
      <div className="home-gentle-track">
        {courses.map((course, index) => (
          ...
          >
            ...
            <span>{course.docCount} {text.documents || "documents"}</span>
          </button>
        ))}
      </div>
    </section>
    ...
    <div className="home-gentle">
      <section className="home-gentle-hero">
        ...
        <div className="home-gentle-hero-content">
          <div className="home-gentle-pill">{text.welcomeBack}, {firstName}</div>
          ...
          <div className="home-gentle-search-row">
            ...
            >
              {model.isBusy ? text.searching : text.search}
            </button>
          </div>
      ...
      <DocumentRowSection
        title={text.continueReading}
        subtitle={text.continueReadingSubtitle}
        docs={model.continueReading}
        icon={<ClockIcon />}
        actionLabel={text.viewLibrary}
        onAction={() => controller.onGoToTab("library")}
        onOpenDoc={controller.onSelectDocumentCard}
      />

      <DocumentRowSection
        title={text.trendingDocuments}
        subtitle={text.trendingSubtitle}
        docs={model.trendingDocs}
        icon={<TrendIcon />}
        actionLabel={text.viewAllTrending}
        onAction={() => controller.onGoToTab("search")}
        onOpenDoc={controller.onSelectDocumentCard}
        rank
      />

      <CourseRowSection courses={model.courseCards.map((course) => ({ ...course, text }))} onOpenCategory={controller.onOpenCategory} />

      <div ref={recentlyRef}>
        <DocumentRowSection
          title={text.recently || (model.user?.language === "en" ? "Recently" : "Mới gần đây")}
          subtitle={text.recentSubtitle || (model.user?.language === "en" ? "Recently added approved materials" : "Tài liệu đã duyệt được thêm gần đây")}
