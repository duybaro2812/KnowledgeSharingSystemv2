...
let qaModerationSchemaReady = false;

const ensureQaModerationSchema = async () => {
    if (qaModerationSchemaReady) return;
    const pool = getPool();
    ...
    await pool.query(`
        ALTER TABLE question_messages
        ADD COLUMN IF NOT EXISTS reviewed_by_user_id INTEGER NULL REFERENCES users(user_id);
    `);
    await pool.query(`
        ALTER TABLE question_messages
...
const getSessionMessages = async ({ sessionId, userId, role = '' }) => {
    await ensureQaModerationSchema();
    const pool = getPool();
    const accessCheck = await getSessionByIdForUser({ sessionId, userId });
    const canSeeHidden = ['admin', 'moderator'].includes(String(role || '').toLowerCase());
...
const reportMessage = async ({ sessionId, messageId, reporterUserId, reason }) => {
    await ensureQaModerationSchema();
    const pool = getPool();

    const accessCheck = await getSessionByIdForUser({ sessionId, userId: reporterUserId });
    if (!accessCheck) {
        const error = new Error('Q&A session not found or no permission.');
        error.statusCode = 404;
        throw error;
    ...
    if (!message) {
        const error = new Error('Q&A message not found.');
        error.statusCode = 404;
        throw error;
    }

    if (Number(message.senderUserId) === Number(reporterUserId)) {
        const error = new Error('You cannot report your own Q&A message.');
        error.statusCode = 400;
        throw error;
    }
    ...
    const reportResult = await pool.query(
        `
            INSERT INTO qa_message_reports (session_id, message_id, reporter_user_id, reason)
            VALUES ($1, $2, $3, $4)
        ...
        `,
        [sessionId, messageId, reporterUserId, reason]
    );

    return {
        ...reportResult.rows[0],
        documentId: accessCheck.documentId,
        message,
    };
};

const setMessageStatus = async ({ sessionId, messageId, moderatorUserId, status, note = null }) => {
    await ensureQaModerationSchema();
    const pool = getPool();

    const result = await pool.query(
        `
            UPDATE question_messages qm
            SET
                status = $4,
            ...
            RETURNING
                ...
                qm.reviewed_at AS "reviewedAt";
        `,
        [sessionId, messageId, moderatorUserId, status, note]
    );
    ...
    await pool.query(
        `
            UPDATE qa_message_reports
            SET
                status = CASE WHEN $4 = 'hidden' THEN 'resolved' ELSE 'dismissed' END,
                reviewed_by_user_id = $3,
                review_note = $5,
                reviewed_at = NOW()
            WHERE session_id = $1
