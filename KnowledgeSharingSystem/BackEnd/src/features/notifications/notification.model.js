...
const inferActionByType = (type) => {
    const map = {
        ...
        document_moderation_pending_review: 'document.pending_review',
        ...
        plagiarism_rechecked: 'document.plagiarism_rechecked',
...
const createNotification = async ({ userId, type, title, message, metadata = null }) => {
    ...
    const preferenceColumnByType = (() => {
        if (normalizedType.includes('comment')) return isPostgresClient() ? 'notify_comments' : 'notifyComments';
        if (
            normalizedType.includes('document_approved') ||
            normalizedType.includes('document_moderation_approved') ||
            normalizedType.includes('document_auto_approved')
        ) {
            return isPostgresClient() ? 'notify_document_approved' : 'notifyDocumentApproved';
        }
        if (normalizedType.includes('qa_session') || normalizedType.includes('qa_message')) {
            return isPostgresClient() ? 'notify_qa_messages' : 'notifyQaMessages';
        }
        if (normalizedType.includes('point')) return isPostgresClient() ? 'notify_points' : 'notifyPoints';
        return '';
    })();

    if (preferenceColumnByType) {
        if (isPostgresClient()) {
            const prefResult = await pool.query(
                `SELECT ${preferenceColumnByType} AS enabled FROM users WHERE user_id = $1;`,
                [userId]
            );
            if (prefResult.rows?.[0]?.enabled === false) return null;
        } else {
            const prefResult = await pool
                .request()
                .input('userId', sql.Int, userId)
                .query(`SELECT ${preferenceColumnByType} AS enabled FROM dbo.Users WHERE userId = @userId;`);
            if (prefResult.recordset?.[0]?.enabled === false || prefResult.recordset?.[0]?.enabled === 0) return null;
        }
    }
