...
const findLatestActiveOtpByEmailAndUsername = async ({ email, username }) => {
    const pool = getPool();
...
const createOrReplaceAdditionalEmailOtp = async ({
    userId,
    email,
    otpCode,
    expiresAt,
    ...
    const pool = getPool();
    const username = `additional_email:${Number(userId)}`;
    const name = 'Additional email verification';
    ...
    if (isPostgresClient()) {
        ...
        try {
            ...
            await client.query(
                ...
                VALUES ($1, $2, $3, $4, $5, $6);
                `,
                [username, name, email, passwordHash, otpCode, expiresAt]
            );
            await client.query('COMMIT');
        } catch (error) {
            await client.query('ROLLBACK');
...
module.exports = {
    ...
    findLatestActiveOtpByEmailAndUsername,
    markOtpAsUsed,
    createOrReplaceForgotPasswordOtp,
    createOrReplaceAdditionalEmailOtp,
};
