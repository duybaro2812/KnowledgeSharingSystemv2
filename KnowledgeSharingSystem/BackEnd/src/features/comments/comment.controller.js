...
const createComment = async (req, res, next) => {
    try {
        ...
        if (String(createdComment?.status || '').toLowerCase() === COMMENT_STATUSES.PENDING) {
            try {
                ...
                await notifyModerators({
                    ...
                    message: `${actorLabel(req.user)} commented on "${documentInfo?.documentTitle || 'document'}".`,
                    metadata: {
                        documentId,
                        commentId,
                        authorUserId: req.user.userId,
                        status: COMMENT_STATUSES.PENDING,
                        action: 'comment.pending_review',
                        target: {
                            type: 'comment',
                            id: commentId,
                        },
                        route: `/moderation?queue=comments&commentId=${commentId}`,
                    },
                });
            } catch (notifyError) {
                console.error('Failed to notify moderators about pending comment:', notifyError.message);
            }

            res.status(201).json({
                success: true,
                message: 'Comment submitted successfully and is pending moderation review.',
                data: createdComment,
            });
            return;
        }
...
const createReplyComment = async (req, res, next) => {
    try {
        ...
        if (String(createdReply?.status || '').toLowerCase() === COMMENT_STATUSES.PENDING) {
            try {
                ...
                await notifyModerators({
                    ...
                    message: `${actorLabel(req.user)} replied in "${documentInfo?.documentTitle || 'document'}".`,
                    metadata: {
                        ...
                        authorUserId: req.user.userId,
                        status: COMMENT_STATUSES.PENDING,
                        action: 'comment.reply_pending_review',
                        target: {
                            type: 'comment',
                            id: commentId,
                        },
                ...
                console.error('Failed to notify moderators about pending reply:', notifyError.message);
            }

            res.status(201).json({
                success: true,
                message: 'Reply submitted successfully and is pending moderation review.',
                data: createdReply,
            });
            return;
        }
