...
router.use((req, res, next) => {
    ...
    const allowPostgresPhase3Paths = [
        ...
        { method: 'GET', test: (path) => /^\/documents\/\d+\/hidden-knowledge$/.test(path) },
        { method: 'PATCH', test: (path) => /^\/documents\/\d+\/hidden-knowledge$/.test(path) },
        { method: 'GET', test: (path) => /^\/documents\/\d+\/hidden-knowledge\/sources$/.test(path) },
        { method: 'POST', test: (path) => /^\/documents\/\d+\/hidden-knowledge\/sources$/.test(path) },
